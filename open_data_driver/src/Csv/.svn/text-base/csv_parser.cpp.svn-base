...
bool csv_parser::init_zip(ZZIP_FILE * input_file_pointer)
{
	input_zzip_fp = input_file_pointer;

	if (input_zzip_fp == NULL)
	{
		fprintf(stderr, "Fatal error : unable to open input file from file pointer\n");
...
csv_row csv_parser::get_row_zip(void)
{
	csv_row current_row;
	...
	/* Grab one record */
	_read_single_line_zip(&line, &line_length);
...
const size_t length_of_row = current_pos - original_pos;
	...
	if (length_of_row > 0)
	{
		...
		*buffer = (char *) realloc(*buffer, *buffer_len);
		...
		original_pos +=length_of_row;
	}
}
	void csv_parser::_read_single_line_zip(char ** buffer, unsigned int * buffer_len)
	{
		ZZIP_FILE *file;
		file = input_zzip_fp;
		//long int original_pos; //= ftell(input_fp);
		//long int current_pos  = original_pos;
		current_pos= original_pos;
		const int limit_dup = 20;
		int r=0, duplicate=0;
		bool not_empty=false;

		//register int* current_char = 0;
		char buf[2];
		char current_char,prec_char=' ';
		/* Checking one character at a time until the end of a line is found */
		while(true)
		{					

			zzip_read(input_zzip_fp,buf,1);   //current_char= fgetc(input_fp);
			current_char = buf[0];
		...
		if (more_rows)
		{
			...
			more_rows = (current_char <= 0 || current_char==127 || duplicate==limit_dup || current_char == line_term_char ) ? false : true;

		}

		/* Find out how long this row is */

	const size_t length_of_row = current_pos - original_pos -r;
		...
		if (length_of_row > 0)
		{
			...
			*buffer = (char *) realloc(*buffer, *buffer_len);
			...
			/* Reset the internal pointer to the original position */
			zzip_seek(input_zzip_fp, original_pos, SEEK_SET); //fseek(input_fp, original_pos, SEEK_SET);

			/* Copy the contents of the line into the buffer */
			zzip_read(input_zzip_fp,*buffer, length_of_row);

			zzip_seek(input_zzip_fp,current_pos, SEEK_SET);



			original_pos +=length_of_row+r;

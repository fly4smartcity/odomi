	...
	var opendata = Array();
	var oi = 0 ; // open data index, since there are multiple query vector opendata is longer than odZone
    ...
    function visualizeOpendata (odObj,h,label) { //visualize an array of open data vectors 
    	...
    	console.log(" open data length", oi);
	}
	...
	function placeMarker(position, map) {
	  ...
	  console.log(path[c].latitude + " " + path[c].longitude + " index " + c);
	...
	{		
		...
		for (var i = 0; i < vectorMarkerWp.length; i++ ) {
			vectorMarkerWp[i].setMap(null);
			if (cx != 0)vectorMarkerWp[i].setMap(null);

			path[i] = null;
		  }
		vectorMarkerWp.length = 0;
		cx = 0;

		console.log(" Erasing open data ");

		for (var i = 0; i < opendata.length; i++ ) {
			opendata[i].setMap(null);
